...
res_assoc['consequents'] = res_assoc['consequents'].map(
    lambda x: set([re.sub("'", '', st) for st in re.sub("[{}]", '', x).split("', ")]))
res_assoc['antecedents'] = res_assoc['antecedents'].map(
    lambda x: set([re.sub("'", '', st) for st in re.sub("[{}]", '', x).split("', ")]))
res_assoc['len_ant'] = res_assoc.antecedents.map(len)
res_assoc['len_cons'] = res_assoc.consequents.map(len)
...
def get_conc_name(k, zapros):
    names = []
    for i, n in enumerate(name_to_id[str(k)]['name_neighbors']):
        if n == zapros:
            name = name_to_id[str(k)]['title_neighbors'][i]
            names.append(name)
    return names
